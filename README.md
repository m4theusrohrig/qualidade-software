# Projeto — Qualidade de Software

## 👥 Integrantes

| Matheus Röhrig Corrêa | 071900826 |

---

## 🎯 Objetivo do Repositório

Este repositório reúne as atividades desenvolvidas na disciplina de **Qualidade de Software**,
realizadas por meio da metodologia PBL, tendo como fio condutor o projeto **Local Eats** —
plataforma de descoberta de restaurantes independentes.

---

## 📚 Organização

```
projeto-qualidade-software/
│
├── README.md
├── docs/                    → Atividades realizadas
│
├── artefatos/               → Diagramas, Relatórios e registros
│
├── src/                     → Código-fonte (quando aplicável)
│
└── referencias/             → Materiais de apoio e bibliografia
```

---

## 📦 Entregas

### Atividade 01

| Atividade     | Descrição                         | Status      |
| ------------- | --------------------------------- | ------------|
| PBL 1         | Diagnóstico Inicial da Qualidade  | ✅ Entregue |
| PBL 2         | Diagnóstico Organizacional de QA  | ✅ Entregue |
| PBL 3         | Estratégia Inicial de Testes      | ✅ Entregue |
| PBL 4         | Testes Funcionais vs Estruturais  | ✅ Entregue |
| PBL 5         | Planejamento e Execução de Testes | ✅ Entregue |

### Atividade 02

| Atividade     | Descrição                                      | Status      |
| ------------- | ---------------------------------------------- | ------------|
| PBL 6         | Testes Unitários Automatizados e TDD           | ✅ Entregue |
| PBL 7         | Testes Funcionais Automatizados                | ✅ Entregue |
| PBL 8         | BDD e automação orientada a comportamento      | ⏳ Pendente |


---

## 🧩 Metodologia

**Aprendizagem Baseada em Problemas (PBL)**, com foco no desenvolvimento de
competências em qualidade de software aplicadas ao projeto **Local Eats**.

O problema norteador simula uma situação real: uma startup lançou rapidamente
uma plataforma digital e precisa de uma equipe de qualidade para diagnosticar
e corrigir seus problemas com base na norma **ISO/IEC 25000**.

---

## ✅ Boas práticas adotadas

- Commits frequentes com mensagens descritivas
- Organização clara das pastas por atividade
- Versionamento de todos os artefatos
- README atualizado a cada nova entrega

---

**Instituição:** Centro Universitário Senac-RS
**Curso:** Análise e Desenvolvimento de Sistemas 
**Unidade Curricular:** Qualidade de Software
**Prof.:** Luciano Zanuz